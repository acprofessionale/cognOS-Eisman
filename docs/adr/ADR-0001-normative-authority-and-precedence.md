...
**Current tenet requiring annotation:** T-04 (`docs/design-tenets.md:76–88`) states: "Every automated behavior must be auditable and reversible by the operator." This statement intersects execution safety. Upon acceptance of this ADR, T-04 must be annotated with the following verified Core references: P1 (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:142–146`), P5 (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:173–191`), P6 (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:193–197`), D-003 (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:1015–1017`), D-008 (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:1035–1037`), D-009 (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:1039–1041`), Constitutional Invariant 1 (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:238`), Core ADR-0002 (`CognOS-Core/docs/architecture/adr/ADR-0002-human-in-the-loop-baseline.md`). No substantive change to T-04 text is required — only the cross-reference annotation.

**Additional tenet requiring annotation:** T-05 (Voice First, `docs/design-tenets.md:92–105`) covers voice as an input channel for CognOS interactions. Core P4 (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:159–171`) establishes that voice carries `channel_trust = untrusted_ambient`, is not authentication, is not authorization, and cannot approve R2–R4 side effects alone. Constitutional Invariant 9 (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:247`) states: "Voice is expressive, not authoritative." T-05 and P4 are compatible and address complementary domains (T-05 governs engineering design; P4 governs runtime channel-trust semantics). However, T-05's subject matter intersects the channel-trust and authorization boundary governed by Core. Upon acceptance of this ADR, T-05 must be annotated with a reference to Core P4 (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:159–171`), D-006 (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:1027–1029`), and Constitutional Invariant 9 (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:247`). No substantive change to T-05 text is required — only the cross-reference annotation.

**CONSTRAINT:** This ADR does not authorize modification of T-04, T-05, or any other tenet. Those actions each require a separate ADR. This paragraph records both obligations and defers their execution to human review.
...
- Specialize Ecosystem tenets for local context (example: specifying voice-first testing procedures specific to CognOS-Eisman's ice cream truck domain, consistent with T-05, `docs/design-tenets.md:93–105`)
...
|---|---|---|---|---|
| Engineering principles | Ecosystem Design Tenets (`docs/design-tenets.md:1–261`) | Vertical Modules may add more specific engineering rules consistent with T-01–T-12 | No vertical may define an alternative principle set that contradicts T-01–T-12 | Ecosystem tenets prevail over vertical-local rules |
...
| V-07 | T-04 (safety-adjacent tenet) contains an explicit Core reference | **NOT MET** — T-04 (`docs/design-tenets.md:76–88`) does not currently contain a Core reference. Cross-reference annotation required before acceptance. Applicable Core references identified: P1, P5, P6, D-003, D-008, D-009, Constitutional Invariant 1, Core ADR-0002 (see Problem Statement and Cross-Domain Obligation Rule sections of this ADR). | **OPEN — requires tenet amendment via separate ADR** |
| V-08 | Authority Matrix has no unresolved ownership overlaps | All 10 matrix rows reviewed; overlaps documented and assigned | **MET WITH ANNOTATION — all 10 rows verified against Core v0.2. [TO BE VERIFIED] markers resolved. One additional finding requiring human decision: Core v0.2 §0 places GOVERNANCE.md and SECURITY.md at level 2 of its hierarchy (`CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md:29`); these are not currently represented in the Eisman Authority Matrix. This is an omission, not a conflict. Resolution deferred to human review.** |
...
- `docs/design-tenets.md` — T-01–T-12, Design Tenets v1.0 July 2026 (lines 1–261)
...
- `CognOS-Core/docs/COGNOS_Architecture_Manifesto_v0.2.md` — authoritative constitutional reference (v0.2, 2026-07-22, 1171 lines)
...
*ADR-0001 — Normative Authority and Governance Precedence*
*Status: Proposed — Human approval required before Accepted*
*Authored: 2026-07-27*
